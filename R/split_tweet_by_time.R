...
#fileOut<-file("/Users/yangkui/Desktop/count.csv",open = "w")
for (i in 1:6){
  count_df<-data.frame(period=paste(date_v,i,sep = "_"),count=sum(tweets$label==i),rate=sum(tweets$label==i)/(4*3600))
  write.table(count_df,file = "/Users/yangkui/Desktop/count.csv",append = TRUE,row.names = FALSE,col.names = FALSE)
  print(count_df)
}
#close(fileOut)
return(0)
